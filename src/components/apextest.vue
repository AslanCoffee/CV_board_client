  ...
  methods: {
    ...
    async fetchKeywordCounts() {
      try {
        ...
        this.charts[0].series = [{ name: 'Редактировано полей', data: seriesData }];
      } catch (error) {
    ...
    async fetchOtherCounts(options, graphIndex) {
      try {
        ...
        this.charts[graphIndex].series = [{ name: this.getOptionsDisplayValue(options), data: seriesData }];
      } catch (error) {
    ...
    allFetch() {
      ...
      this.fetchOtherCounts('STATUS', 2);
      this.fetchOtherCounts('DOCUMENT', 3);
    },
    ...
    getOptionsDisplayValue(options) {
      switch (options) {
        case 'CREATE':
          return 'Создано';
        case 'STATUS':
